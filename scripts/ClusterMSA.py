...
if __name__=='__main__':
    ...
    for clust in clusters:
        ...
        write_fasta(tmp.SequenceName.tolist(), tmp.sequence.tolist(), outfile=args.o+'/'+args.keyword+'_'+"%03d" % clust+'.a3m', querySeq = seqs[0], oligomericState = int(args.n_mer))

    print('writing 10 size-10 uniformly sampled clusters')
    for i in range(args.n_controls):
       tmp = df.sample(n=10)
       tmp = pd.concat([query_, tmp], axis=0)
       write_fasta(tmp.SequenceName.tolist(), tmp.sequence.tolist(), outfile=args.o+'/'+args.keyword+'_U10-'+"%03d" % i +'.a3m', querySeq = seqs[0], oligomericState = int(args.n_mer)) 
    if len(df)>100:
        print('writing 10 size-100 uniformly sampled clusters')
        for i in range(args.n_controls):
            tmp = df.sample(n=100)
            tmp = pd.concat([query_, tmp], axis=0)
            write_fasta(tmp.SequenceName.tolist(), tmp.sequence.tolist(), outfile=args.o+'/'+args.keyword+'_U100-'+"%03d" % i +'.a3m', querySeq = seqs[0], oligomericState = int(args.n_mer))
